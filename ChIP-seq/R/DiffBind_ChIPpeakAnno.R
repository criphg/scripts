...
#args[3] = summit (distancia esperado para o pico de cada fragmento)
...
write.table((as.data.frame(samples)) ,file=paste0(args[3],"_samples.Tab"), sep="\t")
...
write.table((as.data.frame(AnA.DB)) ,file=paste0(args[3],"_DB_analisys.out"), sep="\t")

#8 - MAplot
pdf(paste0(args[3] , "_MAplot.pdf") , width=8, height = 6)
dba.plotMA(AnA)
dev.off()

#9 - BoxPlot

pdf(paste0(args[3] , "_Boxplot.pdf") , width=8, height = 6)
BPlotN = dba.plotBox(AnA)
dev.off()

## movendo arquivos criados para diretório DiffBind
system("mkdir -p DiffBind_D")
...
cat("\nChiIPpeakAnno analysis...\n")
	...
	###PARA NARROW
        ...
        write.table((as.data.frame(AnnoPeak)) ,file=paste0(args[3],".out"), sep="\t")
